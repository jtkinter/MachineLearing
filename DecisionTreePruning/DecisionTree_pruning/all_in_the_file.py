...
def tree_accuracy(root: DecisionTreeNode | None, predict_s: int | np.ndarray, tags: np.ndarray) -> float:
    if len(tags) == 0:
        return 0.0
    if not root is None:
        predict_s = [predict(root, x) for x in predict_s]
    passive_list = (predict_s == tags)
    return np.sum(passive_list) / len(tags)
...
def create_decision_tree(features: np.ndarray, tags: np.ndarray, idx_list: list[int],
    ...
    return core_create_tree(features, tags, idx_list, mask, classifier)
